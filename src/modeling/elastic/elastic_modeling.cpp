...
void Elastic_modeling::set_parameters(std::string file)
{
...
void Elastic_modeling::set_geometry()
{
...
void Elastic_modeling::build_outputs()
{
    if (export_receiver_output)
    {
        ...
        for (int node = 0; node < total_nodes; node++)
        {
            ...
            receiver_output[time_id +  node * total_times] = 0.5f*(Txx[index] + Tzz[index]);
        }
    }
